...
struct synaptics_rmi4_data *get_rmi4_data = NULL;
#include "../../../fih/fih_touch.h"
extern int tp_probe_success;
extern struct fih_touch_cb touch_cb;
static unsigned int double_tap_enable = FIH_DOUBLE_TAP;
static bool IsTouchPowerOn = false;


void synaptics_3708_touch_fwver_read(char *fwver);
...
#ifdef FB_RESUME_WORK_QUEUE
static void synaptics_rmi4_handle_pm(struct work_struct *work);
#endif
...
unsigned int touch_double_tap_read_synaptics(void);
int touch_double_tap_write_synaptics(unsigned int enable);
...
unsigned int synaptics_rmi4_get_config_id(void)
{
    ...
    struct synaptics_rmi4_data *rmi4_data = get_rmi4_data;

    retval_reg = synaptics_rmi4_reg_read(rmi4_data,rmi4_data->f34_ctrl_base_addr,config_id,sizeof(config_id));
    device_config_id = config_id[0]<<24 | config_id[1]<<16 | config_id[2]<<8 | config_id[3];
    ...
    if (retval_reg < 0)
    {
        pr_err("Read touch config fail... retval_reg=%x",retval_reg);
        return 0;
    }
    ...
    for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++)
    {
                ...
                pdt_entry_addr -= PDT_ENTRY_SIZE)
        {
            ...
            case SYNAPTICS_RMI4_F51:
                ...
                retval = synaptics_rmi4_set_intr_mask(fhandler, &rmi_fd,
                                             intr_count);
                if (retval < 0)
                    return retval;
#endif
...
flash_prog_mode:
    ...
    if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
    {
        ...
        rmi4_data->enable_wakeup_gesture = double_tap_enable;
        pr_info("double_tap_enable=%d \n",double_tap_enable);
    }
    else
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    // alex.naidis@paranoidandroid.co Remove INPUT_MT_DIRECT flag
    // to avoid setting BTN_TOUCH keybit.
...
static void synaptics_rmi4_power_on(void)
{
    int retval;
    struct synaptics_rmi4_data *rmi4_data = get_rmi4_data;
    const struct synaptics_dsx_board_data *bdata =
...
static void synaptics_rmi4_power_off(void)
{
    int retval;
    struct synaptics_rmi4_data *rmi4_data = get_rmi4_data;
    ...
    if(IsTouchPowerOn == false)
        return;
    gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);//low
    msleep(2);
    gpio_set_value(bdata->power_gpio, !bdata->power_on_state);
    msleep(2);
    ...
    if (rmi4_data->pwr_reg)
    {
        ...
        if (retval < 0)
        {
            pr_err("##[%s]: Failed to disable power regulator\n",__func__);
            return ;
        }
    }
    IsTouchPowerOn = false;
    return ;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
...
#ifdef FB_RESUME_WORK_QUEUE
    wakeup_source_init(&rmi4_data->ws, "dsx_tp_ws");
    rmi4_data->pm_workqueue= create_singlethread_workqueue("dsx_pm_workqueue");
    INIT_DELAYED_WORK(&rmi4_data->pm_work, synaptics_rmi4_handle_pm);
#endif
    ...
    get_rmi4_data = rmi4_data;
    tp_probe_success = 1;
    touch_cb.touch_tpfwver_read = synaptics_3708_touch_fwver_read;
    touch_cb.touch_double_tap_read = touch_double_tap_read_synaptics;
    touch_cb.touch_double_tap_write = touch_double_tap_write_synaptics;
...
#ifdef FB_RESUME_WORK_QUEUE
    wakeup_source_trash(&rmi4_data->ws);
#endif
...
err_set_input_dev:
    ...
    if (bdata->power_gpio >= 0)
    {
        gpio_set_value(bdata->power_gpio,!bdata->power_on_state);//Low
        synaptics_rmi4_gpio_setup(bdata->power_gpio, false, 0, 0);
    }
...
#ifdef FB_RESUME_WORK_QUEUE
    cancel_delayed_work_sync(&rmi4_data->pm_work);
    flush_workqueue(rmi4_data->pm_workqueue);
    destroy_workqueue(rmi4_data->pm_workqueue);
...
static void synaptics_rmi4_rebuild_fn_list(struct synaptics_rmi4_data *rmi4_data)
{
    int retval;
                                  ...
                                  bool enable)
{
    ...
    pr_err("Not found fn_number \n");

    synaptics_rmi4_rebuild_fn_list(rmi4_data);
...
static inline void synaptics_pm_prepare_queuing(struct synaptics_rmi4_data *rmi4_data) {
    /* While this could add significant delay in theory
       we must ensure sync. Also as there is a natural delay
       between fb notifications, this doesn't delay execution
       for more than 30ms in my testing.
    */
    cancel_delayed_work_sync(&rmi4_data->pm_work);

    /* Rearm ws timer with pessimistic timeout */
    __pm_wakeup_event(&rmi4_data->ws, PM_SMART_WS_TIMEOUT_MS);
}
// alex.naidis@paranoidandroid.co Advanced scheduling and wakeup tuning - end
        ...
        unsigned long event, void *data)
{
    ...
    struct synaptics_rmi4_data *rmi4_data =
        container_of(self, struct synaptics_rmi4_data,dsi_panel_notif);


    if (!evdata || (evdata->id != 0))
        return 0;

    if (rmi4_data && (event == MSM_DRM_EARLY_EVENT_BLANK))
    {
...
#ifdef FB_RESUME_WORK_QUEUE
            synaptics_pm_prepare_queuing(rmi4_data);
            rmi4_data->fb_ready = true;
            queue_delayed_work(rmi4_data->pm_workqueue,
                               &rmi4_data->pm_work,
                               msecs_to_jiffies(RESUME_WORK_DELAY_MS));
#else
    ...
    if (rmi4_data && event == MSM_DRM_EVENT_BLANK)
    {
        blank = *(int *)(evdata->data);
        if (blank == MSM_DRM_BLANK_POWERDOWN)
        {
            ...
            queue_delayed_work(rmi4_data->pm_workqueue,
                               &rmi4_data->pm_work,
                               msecs_to_jiffies(SUSPEND_WORK_DELAY_MS));
#else
...
static void synaptics_rmi4_handle_pm(struct work_struct *work)
{
    ...
    pr_info("%s: Set ts state to %d\n", __func__, rmi4_data->fb_ready);

    /* Cleanly relax ws */
    __pm_relax(&rmi4_data->ws);
}
#endif
// alex.naidis@paranoidandroid.co Advanced scheduling and wakeup tuning - end
...
void synaptics_3708_touch_fwver_read(char *fwver)
{
    ...
    sprintf(fwver, "Synaptics-V%.4X",config_id);
    return;
}
...
unsigned int touch_double_tap_read_synaptics(void)
{
    pr_info("%s, double_tap = %d", __func__, double_tap_enable);
    return get_rmi4_data->enable_wakeup_gesture;
}
int touch_double_tap_write_synaptics(unsigned int enable)
{
    pr_info( "%s, current double_tap = %d, set double_tap = %d", __func__, double_tap_enable, enable);
    double_tap_enable = enable;
    get_rmi4_data->enable_wakeup_gesture = enable;
    return 0;
}
